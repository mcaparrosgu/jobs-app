...
| T95 | 2 · Crítica | Relanzar `npm run evals` completo (las dos llamadas) y confirmar que la puerta ya no da ROJO. **1er intento (24/08): NO CONCLUYENTE** — timeout de Cloudflare en `generarCvYCarta`. Tras subir el timeout (T103), una prueba suelta repitió el mismo fallo Y OpenRouter se quedó sin cupo del día (429 en los dos modelos de respaldo) — **hoy ya no es buen momento**; repetir mañana temprano con los dos cupos renovados | `evals/` | `npm run evals:puerta` sobre el resultado nuevo da VERDE (o al menos ya no las mismas señales de contenido) | T94, T102, T103 | [ ] |
...
| T99 | 6 · Media | Añadir las mismas 2 variables en Vercel (Settings → Environment Variables) — a mano, la CLI no tiene sesión iniciada y las variables son *Sensitive* (nota de T75, `CLAUDE.md`) | — | Producción usa Cloudflare como principal, no cae a OpenRouter en silencio | T97 | [ ] |
