...
> 🔴 **T109 arreglada la tarde del 25/08** — la generación de CV fallaba al
> 100% por la latencia del modelo, no por cupo: ver
> `knowledge/incidente-gemma4-razonamiento-t109.md`. **Lo que queda pendiente
> y bloquea a las usuarias es T108**, que solo puede ejecutar Mar a mano en el
> SQL Editor de Supabase (una línea): sin ella, los CVs ya generados siguen
> siendo invisibles. El planteamiento original de los dos problemas está en
> `knowledge/pendiente-generacion-cv-falla-25-08.md`.
...
| T113 | 3 · Alta | **Confirmar la calidad de la generación**: la tanda del 25/08 salió ROJO por CVs demasiado cortos (6 de 13, entre 125 y 348 car.) y sin saltos de línea (2 de 13) — **ni un solo fallo de invención**, así que el refuerzo de T94 funciona y el problema es el contrario. Se ajustó el prompt el mismo día (regla de qué significa OMITIR + saltos de línea obligatorios) y en los 8 casos que fallaban 2 pasaron (B01 subió de 348 a 468 car.), 1 siguió corto y 5 se perdieron por picos de latencia de Cloudflare. Falta una tanda completa con cuota fresca. Si no basta, el siguiente candidato ya está medido: `llama-4-scout` (6,7 s) | `lib/ia.ts`, `prompts/system.md`, `evals/` | `npm run evals:generar` seguido de `npm run evals:puerta` da VERDE | T109 | [ ] |
