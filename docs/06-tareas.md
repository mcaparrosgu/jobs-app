...
| T115 | Descubierta el 26/08 | Tapar el agujero del robot: el detector de `publicar.yml` compara con **el push anterior**, no con lo que hay publicado, así que un cambio de IA bloqueado por la puerta lo arrastra a producción el siguiente commit inocuo. Se ha usado a sabiendas el 26/08 para desbloquear producción. Arreglo propuesto: preguntar a Vercel qué `sha` está en producción y usarlo como base del `git diff`. Detalle en `knowledge/agujero-robot-cambio-ia-arrastrado.md` | `.github/workflows/publicar.yml`, job `decidir` | Un cambio de IA que no pasó la puerta sigue contando como pendiente en el siguiente push | T110 | [ ] |
...
| T117 | 2 · Crítica | **Medir la tasa de éxito del arreglo de T116 con cuota y proveedor frescos.** Las tandas del 26/08 por la tarde dan 1-2 de 5 con *cualquier* combinación (modelo viejo, nuevo, scout, mistral) porque Cloudflare estaba en muy mal momento — el mismo caso B01 salió en 12,9 s por la mañana y se colgó 181 s una hora después. Con ese ruido, 1/5 y 2/5 no se distinguen. Sin este dato no se puede decir si T116 arregla la generación o solo el bucle | `scripts/medir-latencia-generacion.ts` | Una tanda de 5 casos con el proveedor estable da una tasa claramente mejor que la de antes del arreglo | T116 | [ ] |
