...
> 🔴 **Lo primero de la próxima sesión (25/08 por la tarde, decisión de Mar:
> anotar y no arreglar hoy).** Detalle completo, con el diagnóstico ya hecho
> y por dónde empezar, en
> `knowledge/pendiente-generacion-cv-falla-25-08.md`. **Empezar por T109**:
> T108 es de una línea, pero T109 es el problema de verdad.
...
| T109 | 2 · Crítica | **Diagnosticar por qué falla generar el CV en producción** con una usuaria real, cuota fresca y T99 ya hecha. Es la señal que T95 llevaba dos días sin poder confirmar: ya no es explicable por cupo agotado. Empezar por los **logs de Vercel** (`teamId` y `projectId` en el doc) y por la tabla `metricas_ia` (`motivo_fallo`, `duracion_ms`, `proveedor`) — sin gastar cuota. Distinguir 422 (contenido, mirar `verificarCv`/`guardrails`) de 502 (proveedor, mirar timeout de Cloudflare y `RONDAS_MODELOS`) | `app/api/generar/route.ts`, `lib/ia.ts`, `lib/verificarCv.ts` | Mar puede generar un CV y una carta de una oferta y descargarlos | T108 | [ ] |
